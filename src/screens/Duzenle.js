...
const Profil = ({navigation}) => {
  ...
  return (
    ...
    <Text style={styles.txt}>Şifre:  {getValue('sifre')}</Text>
    ...
    <TouchableOpacity><Text style={{ left:50,position: 'absolute', bottom: 25, fontSize: 25,color:'white', width:100}} onPress={()=>{navigation.goBack()}}>Çıkış</Text></TouchableOpacity>
       <TouchableOpacity><Text style={{ position: 'absolute', bottom: 20,right:15, fontSize: 25,color:'white', width:100}} onPress={()=>{duzenle()}}>Kaydet</Text></TouchableOpacity>
     </View>
 </SafeAreaView>
  );
};
